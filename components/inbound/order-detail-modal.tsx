...
interface OrderDetailModalProps {
  isOpen: boolean
  order: InboundActivityDetail | null
  onClose: () => void
}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{order.receipt_id}</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{order.supplier_name}</p>
            </div>
            <button
              onClick={onClose}
